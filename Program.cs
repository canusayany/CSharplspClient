...
class Program
{
    ...
    {
        // Lua LSP 服务器路径
        var serverPath = @"C:\Users\A\.vscode\extensions\sumneko.lua-3.13.5-win32-x64\server\bin\lua-language-server.exe";
        var workspacePath = @"C:\Users\A\Documents\testLua"; // 工作空间路径
        ...
        var client = LanguageClient.PreInit(options =>
{
    ...
    options.WithInitializationOptions(new Dictionary<string, object>
    {
        ...
        try
        {
            ...
            var semanticTokens = await client.TextDocument.RequestSemanticTokens(semanticTokensParams);
            ...
            RelatedDocumentDiagnosticReport info = await client.TextDocument.RequestDocumentDiagnostic(new DocumentDiagnosticParams
            {
                TextDocument = new TextDocumentIdentifier(Path.Combine(workspacePath, fileName))
